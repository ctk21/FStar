...
let copy_env (en:env_t) = { en with global_cache = BU.mk_ref !en.global_cache}  //Make a copy of all the mutable state of env_t, central place for keeping track of mutable fields in env_t
...
let init_env tcenv = last_env :=  [{bvar_bindings=BU.psmap_empty ();
                                   ...
                                   global_cache = BU.psmap_empty() |> BU.mk_ref}]
...
let recover_caching_and_update_env (env:env_t) (decls:decls_t) :decls_t =
  decls |> List.collect (fun elt ->
    if elt.key = None then [elt]  //not meant to be hashconsed, keep it
    else match BU.psmap_try_find !env.global_cache (elt.key |> BU.must) with
         ...
         | None ->  //no hit, update cache and retain elt
           env.global_cache := BU.psmap_add !env.global_cache (elt.key |> BU.must) elt;
           [elt]
  )
